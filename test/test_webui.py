"""Tests for webui.py — GOST WebUI dashboard.

Tests focus on pure logic functions (no PyWebIO session dependency).
"""
...
from webui import (
    ...
    _downsample, _record_data_points, _make_chart_js,
    DataPoint, _chart_opts_queue, _history,
    _MAX_HISTORY, _DISPLAY_MAX,
)
...
def _reset_webui_globals():
    ...
    w._prev_total_input = 0
    w._prev_total_output = 0
    w._prev_total_time = 0.0
    yield
    _history.clear()
    _chart_opts_queue.clear()
    w._prev_total_input = 0
...
class TestFmtDuration:
    ...
    def test_minutes_and_seconds(self):
        # s=0 omitted when other parts exist
        assert _fmt_duration(60) == "1m"
        assert _fmt_duration(125) == "2m 5s"

    def test_hours(self):
        # m=0 and s=0 omitted when other parts exist
...
class TestFmtTime:
    ...
    def test_localtime(self):
        with patch("webui.time") as mock_time:
...
class TestDownsample:
    ...
    def test_over_limit_first_point(self):
        ...
        # Downsampling uses int(i*step) — last index is int(9*10) = 90, not 99
    ...
    def test_uses_module_default(self):
        pts = [DataPoint(float(i), 1.0, 1.0, 1) for i in range(_DISPLAY_MAX)]
        result = _downsample(pts)
        assert len(result) == _DISPLAY_MAX
...
class TestRecordDataPoints:
    def test_creates_per_port_history(self):
        _record_data_points([_mapping(8080)], _stats())
        assert "tcp/8080" in _history
        assert len(_history["tcp/8080"]) == 1

    def test_creates_total_key(self):
        _record_data_points([_mapping(8080)], _stats())
        assert "__total__" in _history

    def test_first_call_speed_zero(self):
        _record_data_points([_mapping(8080)], _stats(t_in=1000, t_out=2000))
        assert _history["__total__"][0].speed_in == 0.0
        assert _history["__total__"][0].speed_out == 0.0

    def test_second_call_computes_speed(self):
        with patch("webui.time") as t:
            t.time.side_effect = [1000.0, 1010.0]
            _record_data_points([_mapping(8080)], _stats(t_in=1000, t_out=2000, conns=5))
            _record_data_points([_mapping(8080)], _stats(t_in=3000, t_out=5000, conns=10))

        dp = _history["__total__"][1]
    ...
    def test_zero_elapsed_no_crash(self):
        import webui as w
        w._prev_total_input = 1000
        w._prev_total_output = 2000
        w._prev_total_time = 1000.0
        with patch("webui.time") as t:
            t.time.return_value = 1000.0
            _record_data_points([_mapping(8080)], _stats(t_in=2000, t_out=4000))

        assert _history["__total__"][0].speed_in == 0.0

    def test_multiple_mappings(self):
        mappings = [_mapping(8080, "TCP"), _mapping(9090, "UDP")]
        _record_data_points(mappings, _stats())
        assert "tcp/8080" in _history
        assert "udp/9090" in _history
        assert len(_history) == 3

    def test_per_port_metrics(self):
        _record_data_points([_mapping(8080, si=100, so=200, conns=3)], _stats())
        dp = _history["tcp/8080"][0]
    ...
    def test_history_capped(self):
        import webui as w
        saved = w._MAX_HISTORY
        w._MAX_HISTORY = 3
        try:
            for _ in range(5):
                _record_data_points([_mapping(8080)], _stats())
            assert len(_history["tcp/8080"]) == 3
            assert len(_history["__total__"]) == 3
        finally:
            w._MAX_HISTORY = saved

    def test_append_multiple_calls(self):
        for _ in range(5):
            _record_data_points([_mapping(8080)], _stats())
        assert len(_history["tcp/8080"]) == 5

    def test_empty_mappings_creates_total(self):
        _record_data_points([], _stats(conns=7))
        assert _history["__total__"][0].current_conns == 7

    def test_updates_prev_totals(self):
        import webui as w
        _record_data_points([_mapping(8080)], _stats(t_in=500, t_out=1000))
        assert w._prev_total_input == 500
        assert w._prev_total_output == 1000
        assert w._prev_total_time > 0
...
class TestMakeChartJs:
    def test_empty_queue(self):
        assert _make_chart_js() == ""

    def test_clears_queue(self):
        _chart_opts_queue.append(("c1", "{}"))
        _make_chart_js()
        assert _chart_opts_queue == []
    ...
    def test_echarts_init(self):
        _chart_opts_queue.append(("my_chart", '{"xAxis":{}}'))
        js = _make_chart_js()
    ...
    def test_embeds_option(self):
        ...
        _chart_opts_queue.append(("x", opts))
        assert opts in _make_chart_js()

    def test_empty_opts(self):
        _chart_opts_queue.append(("c1", ""))
        assert "c1" in _make_chart_js()
...
class TestBuildEchartsHtml:
    def test_empty_points(self):
        from webui import _build_echarts_html
        assert _build_echarts_html("test", []) == ""

    def test_single_point(self):
        from webui import _build_echarts_html
        assert _build_echarts_html("test", [DataPoint(1.0, 0, 0, 0)]) == ""

    def test_two_points_returns_div(self):
        from webui import _build_echarts_html
        pts = [DataPoint(1000.0, 10.0, 20.0, 1), DataPoint(1010.0, 15.0, 25.0, 2)]
        html = _build_echarts_html("chart_test", pts)
    ...
    def test_queues_opts(self):
        ...
        before = len(_chart_opts_queue)
        _build_echarts_html("chart_q", pts)
        assert len(_chart_opts_queue) == before + 1

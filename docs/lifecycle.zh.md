...
管理后台服务的启动和关闭：STUN 解析、SSDP 响应器和租期清理线程。
...
| 方法 | 返回值 | 说明 |
|---|---|---|
| `shutdown_event`（属性） | `threading.Event` 或 `None` | 用于通知后台线程关闭的事件。`start()` 调用前为 `None`。 |
| `start()` | `threading.Event` | 启动 STUN、SSDP 和租期清理线程。返回 shutdown 事件。 |
...
2. 启动 STUN 客户端（如果已配置）：
   - 创建守护线程立即执行首次 STUN 解析，之后每 24 小时刷新。
   - 通过 `wait_ready()` 等待最多 10 秒，等待首次解析完成。
   - 如果 STUN 超时，记录警告并继续——刷新循环会持续重试。
3. 创建 `threading.Event` 用于关闭信号。
4. 启动 SSDP 响应器线程。
5. 启动租期清理守护线程。
6. 返回 shutdown 事件。
